    ...
    >
      ...
      <div class="p-4">
        ...
        <div class="mb-6 space-y-3">
          <label class="flex items-center space-x-3" v-if="false">
            <input
...
const proceedWithClosure = () => {
  ...
  const tableId = restaurantStore.selectedTable?.id
  const customerId = restaurantStore.selectedTable && restaurantStore.selectedTable.customer
    ? restaurantStore.selectedTable.customer.id
    : undefined
  ...
  restaurantStore
    ...
    .then(async () => {
      if (tableId) {
        await restaurantStore.deleteAllOrdersForTable(tableId)
        restaurantStore.clearDeliveryMapping(tableId, customerId)
      }
